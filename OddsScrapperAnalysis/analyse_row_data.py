...
def plot_histograms(df):
    ...
    pd.DataFrame.hist(df[(df['Bet'] == 1) & (df['HomeOdd'] <= 4)], column='HomeOdd', by='SportId', bins=20)
    pd.DataFrame.hist(df[(df['Bet'] == 2) & (df['AwayOdd'] <= 4)], column='AwayOdd', by='SportId', bins=20)
    plt.show()


if __name__ == '__main__':
    #db_data = pd.read_csv('archive.csv')
    db_data = pd.read_csv('test_predictions.csv')
