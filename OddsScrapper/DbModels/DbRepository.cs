...
namespace OddsScrapper.DbModels
{
    public class DbRepository
    {
        ...
        {
            var getCommand = $"SELECT Id FROM {CountriesTable} WHERE Name='{country}';";
        ...
        {
            var getCommand = $"SELECT Id FROM {TeamsTable} WHERE Name='{team}' AND LeagueId='{leagueId}';";
            var insertCommand = $"INSERT INTO {TeamsTable} (Name,LeagueId) VALUES ('{team}','{leagueId}');";

            return GetOrCreateId(getCommand, insertCommand);
        }

        internal int GetOrCreateLeagueId(int sportId, int countryId, LeagueInfo leagueInfo)
        {
            ...
            var getCommand = $"SELECT Id FROM {LeaguesTable} WHERE Name='{name}' AND SportId='{sportId}' AND CountryId='{countryId}';";
        ...
        internal void InsertGames(int leagueId, int season, IEnumerable<Game> games)
        {
